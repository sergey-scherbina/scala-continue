...
object Pipes2 extends App {
  ...
  final case class Cont[A, S, R](cont: ![(A =>> S) =>> R]) extends AnyVal {
    ...
    @inline def map[B](f: A => B): Cont[B, S, R] = Cont(done(k => cont.flatMap(_ (a => k(f(a))))))
  ...
  def runPipeIO[I: Read, O](p: Pipe[I, O, Unit])(r: BufferedReader) = {
    ...
    p.cont.result(_ => done(_ => _ => done(()))).result(ki)(ko).result
  }
